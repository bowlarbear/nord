...
class Saving extends StatefulWidget {
  ...
  @override
  State<Saving> createState() => _SpendingState();
}

class _SpendingState extends State<Saving> with TickerProviderStateMixin {
  ...
  void onPageLoad() async {
    var premiumMember = await checkForPremium();
    if (premiumMember == false) {
      //send user to intial setup flow
      Navigator.push(
        context,
        MaterialPageRoute(
  ...
  Future<bool> checkForPremium() async {
    //if not a premium member, return false
    return false;
    //TODO add
  }
